...
CREATE TABLE quality_inspection_record (
    ...
    inspector BIGINT UNSIGNED NOT NULL COMMENT '检验员',
...
-- 产品1（A-1001）
('PRC-001', '压铸成型', 1, 1, 1, 45, '["表面无气泡","尺寸公差±0.1mm"]'),
('PRC-002', '电路板组装', 1, 2, 3, 120, '["电路导通测试","元件焊接质量"]'),
('PRC-003', '气密测试', 1, 3, 2, 60, '["泄漏率<0.5Pa/min"]'),

-- 产品2（B-2002）
('PRC-004', '定子绕线', 2, 1, 4, 180, '["线圈电阻值","绝缘耐压测试"]'),
('PRC-005', '转子动平衡', 2, 2, 3, 90, '["不平衡量<0.5g·mm"]'),
('PRC-006', '整机装配', 2, 3, 2, 150, '["轴向间隙<0.1mm"]'),

-- 产品3（C-3003）
('PRC-007', '机械臂铸造', 3, 1, 1, 240, '["内部无砂眼","重量公差±10g"]'),
('PRC-008', '精密加工', 3, 2, 3, 300, '["表面粗糙度Ra0.8","尺寸精度±0.01mm"]'),
('PRC-009', '关节组装', 3, 3, 2, 180, '["回转间隙<0.05mm"]');
...
-- 插入生产记录（包含正常/异常情况）
INSERT INTO production_record (order_id, process_id, equipment_id, output_quantity, defective_quantity, start_time, end_time, operator, remark) VALUES
-- 工单1的工序1记录
(1, 1, 1, 200, 2, '2024-01-05 08:32:00', '2024-01-05 12:00:00', 14, '正常生产'),
(1, 1, 1, 180, 1, '2024-01-05 13:30:00', '2024-01-05 15:55:00', 14, '换模时间较长'),

-- 工单2的工序3记录
(2, 3, 2, 500, 3, '2024-01-19 08:10:00', '2024-01-19 11:30:00', 15, '气密检测合格率98%'),
(2, 3, 2, 300, 2, '2024-01-19 13:00:00', '2024-01-19 14:55:00', 15, '设备短暂报警'),

-- 工单6的紧急订单记录
(6, 7, 1, 50, 0, '2024-01-10 07:30:00', '2024-01-10 15:00:00', 14, '提前完成铸造'),
(6, 8, 3, 48, 1, '2024-01-11 08:00:00', '2024-01-11 18:00:00', 15, '刀具异常磨损');
...
-- 插入质检记录（包含合格/不合格记录）
INSERT INTO quality_inspection_record (order_id, item_id, inspection_result, inspection_data, inspector, inspection_time, remark) VALUES
-- 工单1的检测记录
(1, 1, 1, '{"检测结果":"表面光洁，无缺陷"}', 18, '2024-01-05 16:30:00', '首件检验合格'),
(1, 3, 2, '{"测试压力":0.5,"泄漏率":0.8}', 19, '2024-01-08 10:15:00', '密封圈需要更换'),

-- 工单2的检测记录
(2, 4, 1, '{"测试电压":1500,"持续时间":60}', 18, '2024-01-19 15:30:00', '全部通过测试'),
(2, 5, 3, '{"实测电流":3.5}', 19, '2024-01-20 09:00:00', '需二次抽检'),

-- 工单6的检测记录
(6, 7, 1, '{"实测尺寸":"299.98mm"}', 18, '2024-01-14 14:00:00', '精度达标'),
(6, 9, 1, '{"运行时长":72,"故障次数":1}', 19, '2024-01-15 10:30:00', '通过耐久测试');